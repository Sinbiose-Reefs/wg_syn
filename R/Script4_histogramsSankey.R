...
# occurrence (studies that sums per species probability of site occupancy to obtain species richness or occurrence per functional group)
...
df_approach_trait$approach <- factor (df_approach_trait$approach,
                                      levels = c("compare","combine", "both"))

# stacked grouped barplot rather than sankey plot?
# Stacked + percent
pdf(here ("output", "barplot_fig4.pdf"),height = 5,width=5)

ggplot(df_approach_trait, aes(fill=trait, y=value, x=approach)) + 
  geom_bar(position="fill", stat="identity") + 
  scale_fill_viridis_d(option="magma", direction=-1, begin=0.1) + 
  theme_bw() + 
